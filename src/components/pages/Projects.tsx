    ...
    return(
        <div className="h-full overflow-x-hidden min-sm:overflow-y-hidden">
            ...
            <div className={`text-center duration-600 ease-linear py-6 ${translateText ? 'translate-y-0' : '-translate-y-14'}`}>
                <h2 className={`text-lg md:text-2xl uppercase font-semibold ${translateText ? '' : 'text-gray-900'}`}>Projects</h2>
            </div>
            <div className="flex flex-col gap-4 items-center h-full md:w-full my-4 px-8">
                <div className={`duration-600 ease-linear ${translateText ? 'translate-x-0' : '-translate-x-14'}`}>
                    <div className={`bg-black/45 p-4 rounded flex flex-col md:flex-row gap-8 ${translateText ? '' : 'hidden'}`}>
                            <div>
